...
class CGETransModel:
    ...
    def loadDataset(self):
        ...
        for i in range(len(NAMES)):
            if self.dataset[0] == 'n':
                ...
                OBJECTS.append(cPickle.load(open("data/trans.{}.{}".format(self.dataset, NAMES[i]))))
    ...
    def buildModel(self):
        ...
        g_input = lasagne.layers.InputLayer(shape = (None, 2), input_var = g_init)
        ind_input = lasagne.layers.InputLayer(shape = (None, ), input_var = ind_init)
        gy_input = lasagne.layers.InputLayer(shape = (None, ), input_var = gy_init)
        pair_second = lasagne.layers.SliceLayer(g_input, indices = 1, axis = 1)
        pair_first = lasagne.layers.SliceLayer(g_input, indices=0, axis=1)
        pair_first_emd = lasagne.layers.EmbeddingLayer(pair_first, input_size=self.num_ver, output_size = self.embedding_size)
        emd_to_numver = layers.DenseLayer(pair_first_emd, self.num_ver, nonlinearity = lasagne.nonlinearities.softmax)
        index_emd = lasagne.layers.EmbeddingLayer(ind_input, input_size = self.num_ver, output_size = self.embedding_size, W = pair_first_emd.W)
        x_to_ydim = layers.SparseLayer(x_input, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
        index_emd = layers.DenseLayer(index_emd, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
        concat_two = lasagne.layers.ConcatLayer([x_to_ydim, index_emd], axis = 1)
        concat_two = layers.DenseLayer(concat_two, self.y.shape[1], nonlinearity = lasagne.nonlinearities.softmax)
        concat_two_output = lasagne.layers.get_output(concat_two)
        step_loss = lasagne.objectives.categorical_crossentropy(concat_two_output, y_init).mean()
        hid_loss = lasagne.layers.get_output(x_to_ydim)
        step_loss += lasagne.objectives.categorical_crossentropy(hid_loss, y_init).mean()
        emd_loss = lasagne.layers.get_output(index_emd)
        step_loss += lasagne.objectives.categorical_crossentropy(emd_loss, y_init).mean()
        step_params = [index_emd.W, index_emd.b, x_to_ydim.W, x_to_ydim.b, concat_two.W, concat_two.b]
        step_updates = lasagne.updates.sgd(step_loss, step_params, learning_rate = self.step_learning_rate)
        self.step_train = theano.function([x_init, y_init, ind_init], step_loss, updates = step_updates, on_unused_input = 'ignore')
        self.test_fn = theano.function([x_init, ind_init], concat_two_output, on_unused_input = 'ignore')

        # supervised train
        fc_output = lasagne.layers.get_output(emd_to_numver)
        pair_second_output = lasagne.layers.get_output(pair_second)
        sup_loss = lasagne.objectives.categorical_crossentropy(fc_output, pair_second_output).sum()
        sup_params = lasagne.layers.get_all_params(emd_to_numver, trainable = True)
        sup_updates = lasagne.updates.sgd(sup_loss, sup_params, learning_rate = self.sup_learning_rate)
        self.sup_train = theano.function([g_init], sup_loss, updates = sup_updates, on_unused_input = 'ignore')


        cross_entropy = lasagne.objectives.categorical_crossentropy(fc_output, pair_second_output)#.sum()
        cross_entropy = T.reshape(cross_entropy, (1,self.unsup_batch_size), ndim=None)

        mask_input = lasagne.layers.InputLayer(shape = (None, self.window_size+1), input_var = mask_init)
        subPath_in = lasagne.layers.InputLayer(shape = (None, self.window_size+1), input_var = sub_path_init)
        sub_path_emd = lasagne.layers.EmbeddingLayer(subPath_in, input_size = self.num_ver, output_size = self.embedding_size, W = pair_first_emd.W)

        lstm_layer = lasagne.layers.LSTMLayer(sub_path_emd,
        ...
        lstm_params_all = lasagne.layers.get_all_params(lstm_layer, trainable = True)
                ...
                for k in ind[i: j]:
                    ...
                    for l in range(len(path)):
                        for m in range(l - self.window_size, l + self.window_size + 1):
                            ...
                            sub_paths.append(s)
                            mask.append(m)
                yield np.array(g, dtype = np.int32), np.array(sub_paths, dtype = np.int32), np.array(mask, dtype = np.float32)
                i = j
    ...
    def test(self):
        ...
        pred_vali = self.test_fn(self.tx[0 : self.mid], np.arange(self.x.shape[0], self.x.shape[0] + self.mid, dtype = np.int32))
        acc_vali = compAcc(pred_vali, self.ty[0 : self.mid])
        pred_test = self.test_fn(self.tx[self.mid : self.tx.shape[0]], np.arange(self.x.shape[0] + self.mid, self.x.shape[0] + self.tx.shape[0], dtype = np.int32))
...
if __name__ == '__main__':
    ...
    m.loadDataset()
    m.buildModel()
    m.supervisedTrain(20)
    m.unsupervisedTrain(200)
